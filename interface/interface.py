...
class App():
    def __init__(self, find_in_text, place_order):
        ...
        self.stop_notes_entry = ctk.CTkEntry(self.app, width=400, corner_radius=5)  # Make the entry widget wider
        self.stop_notes_entry.grid(row=1, column=1, padx=10, pady=5, sticky='w')
        ...
        self.order_reference_entry = ctk.CTkEntry(self.app, width=400, corner_radius=5)  # Make the entry widget wider
        self.order_reference_entry.grid(row=2, column=1, padx=10, pady=5, sticky='w')
        ...
        self.time_ready_entry = ctk.CTkEntry(self.app, width=400, corner_radius=5, placeholder_text="!!FORMAT: 04:00:PM!!")  # Make the entry widget wider
        self.time_ready_entry.grid(row=4, column=1, padx=10, pady=5, sticky='w')


        self.browse_button = ctk.CTkButton(self.app, text="Browse File", command=lambda:self.browse_file(), corner_radius=5)
        self.browse_button.grid(row=5, columnspan=2, padx=10, pady=5)
        ...
        self.patient_name.grid(row=6, columnspan=2)

        self.file_preview_label = ctk.CTkLabel(self.app, text="Selected File: None")
        self.file_preview_label.grid(row=7, columnspan=2, padx=10, pady=5)

        self.pdf_content_text = ctk.CTkTextbox(self.app, height=150, width=500, corner_radius=5)
        self.pdf_content_text.grid(row=8, columnspan=2, padx=10, pady=5)
        ...
        self.place_order_button.grid(row=10, columnspan=2, padx=10, pady=10)
    ...
    def place_order_and_print(self, data):
        ...
        data["time_ready"] = self.time_ready_entry.get()
